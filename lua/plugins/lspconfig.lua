    ...
    cmd = { "LspInfo", "LspStart" },
      ...
      local capabilities = require('blink.cmp').get_lsp_capabilities()
          ...
          local diag = vim.diagnostic
					vim.keymap.set("n", "[d", function() diag.jump({count = 1, float = true}) end, { desc = "Next Diagnostic" })
					vim.keymap.set("n", "]d", function() diag.jump({count = -1, float = true}) end, { desc = "Previous Diagnostic" })
				end,
			})
		end,
	},
